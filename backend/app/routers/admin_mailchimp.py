...
def trigger_manual_sync(background_tasks: BackgroundTasks, db: Session = Depends(get_db), current_admin = Depends(get_current_admin)):
...
def get_mailchimp_stats(db: Session = Depends(get_db), current_admin = Depends(get_current_admin)):
