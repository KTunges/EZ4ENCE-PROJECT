  ...
  if (!isAuthenticated || !user) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100vh', background: 'var(--bg-dark)', color: 'white', textAlign: 'center', padding: '20px' }}>
        ...
        <p className="text-muted mt-4 mb-6">Bạn cần đăng nhập ở Trang Chủ trước khi vào Admin Dashboard.</p>
  ...
  if (user.role !== 'ADMIN') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100vh', background: 'var(--bg-dark)', color: 'white', textAlign: 'center', padding: '20px' }}>
        ...
        <p className="text-muted mt-4 mb-6">Bạn không có quyền hạn để truy cập vào khu vực này.</p>
