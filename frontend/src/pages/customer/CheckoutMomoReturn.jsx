...
export default function CheckoutMomoReturn() {
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/payment/momo/verify-return${location.search}`);
        ...
        console.error(err);
        setError("Lỗi kết nối khi xác thực giao dịch Momo.");
      }
    };
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center p-4 fade-in">
        <div className="glass max-w-md w-full p-8 text-center border-t-4 border-pink">
          ...
          <h2 className="text-2xl font-bold mb-4 text-white">Thanh Toán Momo Thất Bại</h2>
  ...
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center fade-in">
      ...
      <h2 className="text-xl font-bold text-white mb-2">Đang xác thực thanh toán Momo...</h2>
